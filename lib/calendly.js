...
/**
 * Look up a scheduled event's real start/end time by its Calendly API URI
 * (e.g. the `event.uri` given in the embed's `calendly.event_scheduled`
 * postMessage payload) — used by the public booking wizard to read back
 * the actual slot a visitor picked in the embedded widget, since the
 * postMessage itself doesn't include the plain start time.
 */
async function getScheduledEvent(eventUri) {
  if (!eventUri) throw new Error('eventUri is required');
  const token = process.env.CALENDLY_API_TOKEN;
  if (!token) throw new Error('CALENDLY_API_TOKEN is not configured');
  const r = await fetch(eventUri, { headers: { Authorization: `Bearer ${token}` } });
  const data = await r.json();
  if (!r.ok) throw new Error(data.message || `Calendly event lookup failed with status ${r.status}`);
  return { startTime: data.resource.start_time, endTime: data.resource.end_time };
}

module.exports = { createSchedulingLink, verifyWebhookSignature, parseInviteeCreatedPayload, getScheduledEvent };
