...
function parseBookingPayload(payload) {
  ...
  return {
    ...
    // SCRUM-77: the tutor is the Cal.com account owner/organizer of their
    // own event — used to reconcile a booking back to `tutors` by email
    // when there's no trackingId to look up a lead by instead.
    organizerEmail: payload.organizer && payload.organizer.email,
    // Best-effort, NOT verified against a live webhook delivery — Cal.com
    // generates a real per-booking meeting link (e.g. via its Google
    // Calendar/Meet integration) that is genuinely different from this
    // app's own static tutors.meet_link/MEET_LINK_* fallback, and no field
    // name for it has been confirmed live yet. Tries the most likely
    // locations in the documented v2 payload shape, in order, and falls
    // back to undefined (callers keep using the static link) rather than
...
function extractMeetingLink(payload) {
  if (payload.videoCallData && payload.videoCallData.url) return payload.videoCallData.url;
  if (payload.metadata && payload.metadata.videoCallUrl) return payload.metadata.videoCallUrl;
  if (typeof payload.location === 'string' && /^https?:\/\//.test(payload.location)) return payload.location;
  return undefined;
}
