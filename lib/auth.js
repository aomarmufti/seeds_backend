...
// Sends 401 and returns null if the caller isn't authenticated at all
// (any role); otherwise returns the authed user. For endpoints that are
// self-service (a parent/tutor acting on their own data) rather than
// admin-only — callers still need to check the returned user owns the
// specific record being accessed.
...
module.exports = { getAuthedUser, requireAdmin, requireAuth };
