...
function uid() {
  return `${Date.now()}-${Math.random().toString(36).slice(2)}@seedsinstitute.co.uk`;
}
...
function generateICS(lesson) {
  ...
  const description = isGroup
    ? [
        ...
        ``,
        `Seeds Tuition — seedsinstitute.co.uk`,
      ].join('\n')
    : [
        `1:1 lesson — ${subject}`,
        `Tutor: ${tutorName}`,
  ...
  const organizerEmail = process.env.EMAIL_FROM || 'lessons@seedsinstitute.co.uk';
