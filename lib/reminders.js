...
<body>
  <div class="wrap">
    ...
    <div class="footer">Seeds Tuition Ltd · seedsinstitute.co.uk · If you have questions, reply to this email.</div>
  </div>
</body>
</html>`;
...
<body>
  <div class="wrap">
    ...
    <div class="footer">Seeds Tuition · seedsinstitute.co.uk</div>
  </div>
</body>
</html>`;
...
<body style="font-family:-apple-system,Arial,sans-serif;background:#f5f5f0;margin:0;padding:24px">
  <div style="max-width:540px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden">
    ...
    <div style="padding:28px 32px">
      ...
      ${slotRows}
      <a href="${portalUrl || process.env.FRONTEND_URL || 'https://seedsinstitute.co.uk'}" style="display:block;background:#0D1B2A;color:#fff;text-decoration:none;text-align:center;padding:14px 20px;border-radius:10px;font-weight:700;font-size:15px;margin:20px 0 16px">Pick your time in the portal →</a>
    ...
    <div style="background:#F5F0E8;padding:16px 32px;font-size:12px;color:#A7A7A7;text-align:center">Seeds Tuition · seedsinstitute.co.uk</div>
  </div>
</body>
</html>`;
...
<div style="background:#F5F0E8;padding:14px 28px;font-size:11px;color:#A7A7A7;text-align:center">
  Seeds Tuition · seedsinstitute.co.uk · Unsubscribe preferences in your portal
</div>`;
    ...
    html: `<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;background:#f5f5f0;margin:0;padding:20px">
      <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden">
        ...
        <div style="padding:24px 28px">
          ...
          <p style="color:#4A5568;font-size:14px;line-height:1.6">While you wait, feel free to explore our <a href="https://seedsinstitute.co.uk" style="color:#C8A15A">website</a> to learn more about our tutors and approach.</p>
...
/**
 * Generic admin alert for a critical failure (payment, payout, webhook
 * processing) — used by lib/logger.js's alertCritical rather than letting
 * these classes of error sit as only a console line no one is watching
 * (SCRUM-39).
 */
async function sendAdminAlert({ subject, details }) {
  const transporter = getMailTransport();
  if (!process.env.ADMIN_EMAIL) console.error('ADMIN_EMAIL is not set — critical alert falling back to a hardcoded personal address');
  const adminEmail = process.env.ADMIN_EMAIL || 'azeemomar-mufti@outlook.com';
  await transporter.sendMail({
    from: `"Seeds Tuition Alerts" <${process.env.EMAIL_FROM}>`,
    to: adminEmail,
    subject: `⚠️ ${subject}`,
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;padding:20px">
      <h2 style="color:#c0392b;font-family:Georgia,serif">⚠️ ${escapeHtml(subject)}</h2>
      <pre style="background:#f8f8f8;padding:12px;border-radius:8px;white-space:pre-wrap;font-size:13px;color:#0D1B2A">${escapeHtml(details)}</pre>
    </div>`,
  });
}
...
/**
 * Sent right after a tutor is assigned, when that tutor has Cal.com set
 * up — invites the family to pick a slot on the tutor's Cal.com page.
 */
async function sendSchedulingLink({ parentName, parentEmail, tutorName, subject, schedulingUrl }) {
...
module.exports = {
  ...
  sendPayoutNotification, sendAdminEnquiryAlert, sendAdminAlert,
  sendSchedulingLink,
};
